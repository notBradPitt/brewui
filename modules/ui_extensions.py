...
def save_config_state(name):
    ...
    print(f"Saving backup of brewui/extension state to {filename}.")
    with open(filename, "w", encoding="utf-8") as f:
        json.dump(current_config_state, f, indent=4)
    config_states.list_config_states()
    new_value = next(iter(config_states.all_config_states.keys()), "Current")
    new_choices = ["Current"] + list(config_states.all_config_states.keys())
    return gr.Dropdown.update(value=new_value, choices=new_choices), f"<span>Saved current brewui/extension state to \"{filename}\"</span>"
...
def restore_config_state(confirmed, config_state_name, restore_type):
    ...
    print(f"*** Restoring brewui state from backup: {restore_type} ***")

    if restore_type == "extensions" or restore_type == "both":
        shared.opts.restore_config_state_file = config_state["filepath"]
        shared.opts.save(shared.config_filename)

    if restore_type == "brewui" or restore_type == "both":
        config_states.restore_brewui_config(config_state)
...
def update_config_states_table(state_name):
    ...
    try:
        brewui_remote = config_state["brewui"]["remote"] or ""
        brewui_branch = config_state["brewui"]["branch"]
        brewui_commit_hash = config_state["brewui"]["commit_hash"] or "<unknown>"
        brewui_commit_date = config_state["brewui"]["commit_date"]
        if brewui_commit_date:
            brewui_commit_date = time.asctime(time.gmtime(brewui_commit_date))
        else:
            brewui_commit_date = "<unknown>"

        remote = f"""<a href="{html.escape(brewui_remote)}" target="_blank">{html.escape(brewui_remote or '')}</a>"""
        commit_link = make_commit_link(brewui_commit_hash, brewui_remote)
        date_link = make_commit_link(brewui_commit_hash, brewui_remote, brewui_commit_date)

        current_brewui = config_states.get_brewui_config()

        style_remote = ""
        style_branch = ""
        style_commit = ""
        if current_brewui["remote"] != brewui_remote:
            style_remote = STYLE_PRIMARY
        if current_brewui["branch"] != brewui_branch:
            style_branch = STYLE_PRIMARY
        if current_brewui["commit_hash"] != brewui_commit_hash:
...
<table id="config_state_brewui">
    ...
    <tbody>
        <tr>
            ...
            <td>
                <label{style_branch}>{brewui_branch}</label>
            </td>
            <td>
        ...
        with gr.Tabs(elem_id="tabs_extensions"):
            ...
            with gr.TabItem("Available", id="available"):
                with gr.Row():
                    ...
                    extensions_index_url = os.environ.get('BREWUI_EXTENSIONS_INDEX', "https://raw.githubusercontent.com/AUTOMATIC1111/stable-diffusion-brewui-extensions/master/index.json")
            ...
            with gr.TabItem("Backup/Restore"):
                with gr.Row(elem_id="extensions_backup_top_row"):
                    ...
                    config_restore_type = gr.Radio(label="State to restore", choices=["extensions", "brewui", "both"], value="extensions", elem_id="extension_backup_restore_type")
