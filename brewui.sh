...
# change the variables in brewui-user.sh instead #
#################################################
...
# If run from macOS, load defaults from brewui-macos-env.sh
if [[ "$OSTYPE" == "darwin"* ]]; then
    if [[ -f "$SCRIPT_DIR"/brewui-macos-env.sh ]]
        then
        source "$SCRIPT_DIR"/brewui-macos-env.sh
    fi
fi

# Read variables from brewui-user.sh
# shellcheck source=/dev/null
if [[ -f "$SCRIPT_DIR"/brewui-user.sh ]]
then
    source "$SCRIPT_DIR"/brewui-user.sh
fi
...
# Name of the subdirectory (defaults to stable-diffusion-brewui)
if [[ -z "${clone_dir}" ]]
then
    clone_dir="stable-diffusion-brewui"
fi
...
# read any command line flags to the brewui.sh script
...
then
    ...
    printf "\n%s\n" "${delimiter}"
    printf "Clone stable-diffusion-brewui"
    printf "\n%s\n" "${delimiter}"
    "${GIT}" clone https://github.com/AUTOMATIC1111/stable-diffusion-brewui.git "${clone_dir}"
    cd "${clone_dir}"/ || { printf "\e[1m\e[31mERROR: Can't cd to %s/%s/, aborting...\e[0m" "${install_dir}" "${clone_dir}"; exit 1; }
fi
...
KEEP_GOING=1
export SD_BREWUI_RESTART=tmp/restart
